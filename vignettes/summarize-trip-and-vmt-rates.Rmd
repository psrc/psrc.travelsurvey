...
Because count is an implicit trip attribute, it requires no additional variables.
...
vars <- c("hhsize", "telecommute_freq", "workplace",
...
rs1 <- psrc_hts_triprate(hts_data, group_vars="hhsize")

head(dplyr::select(rs1[survey_year==2023], -c(survey_year, min, max, median)))

# Calculate a VMT rate
rs2 <- psrc_hts_vmtrate(hts_data, group_vars="telecommute_trichotomy")

head(dplyr::select(rs2[survey_year==2023], -c(survey_year, min, max, median)))
```
