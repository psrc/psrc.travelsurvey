...
hhts_varsearch <- function(regex, ...) {
  ...
  {
    varsql <- paste(
      "SELECT [variable] AS var_name, description,",
  ...
  rs <-
    DBI::dbGetQuery(db_connection, DBI::SQL(varsql)) %>% setDT() %>%
    .[grepl(regex, description, ignore.case = TRUE) |
        grepl(regex, var_name, ignore.case = TRUE)] %>% unique()
...
get_var_defs <- function(vars, ...) {
  if (config::is_active('default')) {
    var_def_sql <-
      paste(
        "SELECT [variable] AS var_name, base_table_type",
        "FROM HHSurvey.variable_metadata2;"
      )
  }
  else{
  ...
  db_connection <- hhts_connect(...)
  var_defs <-
    DBI::dbGetQuery(db_connection, DBI::SQL(var_def_sql)) %>% setDT() %>% .[var_name %in% vars]
  DBI::dbDisconnect(db_connection)
  return(var_defs)
}
...
get_hhts <- function(survey, level, vars, ...) {
  dyears <-
    if (survey %in% (c("2017", "2019", "2017_2019", "2021"))) {
  ...
  if (config::is_active('default')) {
    ...
    sql_tbl_ref <-
      sql_hhts_lookup[abbr == level, .(tbl_ref)][[1]]# Convert level to view name
  ...
  db_connection <- hhts_connect(...)
  df <-
    DBI::dbGetQuery(db_connection, DBI::SQL(sql_code)) %>% setDT()                           # Get first row to have column names
  want_vars <-
    grep(wgt_str, colnames(df), value = TRUE) %>% unlist() %>% c(unlist(vars), .)        # Determine available weights
  sql_code <- paste0(
    "SELECT '",
    survey,
    "' AS survey, ",
    paste(want_vars, collapse = ", "),
    " FROM ",
    sql_tbl_ref,
    # Build query for only relevant variables
    " WHERE survey_year IN(",
    paste(unique(dyears), collapse = ", "),
    ");"
  )
  df <-
    DBI::dbGetQuery(db_connection, DBI::SQL(sql_code)) %>% setDT() %>%                       # Retrieve table by year/s
    hhts_recode_na() %>% setDF()                                                                 # Recode NA
  is.na(df) <-
    is.null(df)                                                                       # Recode NULL
  DBI::dbDisconnect(db_connection)
  return(df)
}
...
hhts2srvyr <- function(df, survey, vars, spec_wgt = NULL) {
  dyear <-
    stringr::str_split(survey, "_") %>% lapply(as.integer) %>% unlist()
  var_defs <- get_var_defs(vars) %>% setDT() %>% setkeyv("var_name")
  tbl_names <-
    copy(var_defs) %>% .[var_name %in% vars] %>% .$base_table_type %>% unique()         # Standard weighting by table; construct w/ rules
  if (!is.null(spec_wgt)) {
    wgt_var <-
      spec_wgt                                                                            # Option for power-users to determine the expansion weight
  } else{
    ph_vars <- c(
      "age",
      "age_category",
    ...
    prefix <- unique(
      case_when(
        ...
        any(c("person", "day") %in% tbl_names) &
          grepl("2021", survey) &
          all(vars %not_in% ph_vars) ~ "person",
        TRUE ~ "hh"
      )
    )
    suffix <- case_when(
      grepl("2021", survey) ~ "_ABS",
      mean(dyear) %between% c(2017, 2019) ~ "_v2021",
      TRUE ~ ""
    )
    suffix <-
      case_when(
        grepl("2021", survey) & any(c("person", "day") %in% tbl_names) &
          any(
            grepl(
              "^employment_change_|^workplace|_freq_pre_covid|_mode_pre_covid",
              colnames(df)
            )
          ) ~ paste0(suffix, "_Panel_respondent"),
        grepl("2021", survey) &
          any(grepl("person|trip|day", tbl_names)) &
          grepl("2021", survey) &
          all(vars %not_in% ph_vars) ~ paste0(suffix, "_Panel_adult"),
        "trip" %in% tbl_names &
          survey == "2017_2019" ~ paste0(suffix, "_adult"),
        TRUE ~ suffix
      )
    yearz <- paste0(dyear, collapse = "_")
    wgt_var <-
      paste0(prefix, "_weight_", yearz, suffix)                                           # Otherwise weight determined by rules
  }
  keep_vars <- c("survey", unlist(vars), wgt_var)
  df2 <-
    copy(df) %>% setDT() %>% .[get(wgt_var) > 0, colnames(.) %in% keep_vars, with =
                                 FALSE]        # Keep only necessary elements/records
  ...
  if (!is_empty(ftr_vars)) {
    # srvyr package requires grouping variables as factors;
    for (f in ftr_vars) {
...
hhts_stat <-
  function(df,
           ...
           incl_na = TRUE) {
    vars <-
      c(geographic_unit, stat_var, unlist(group_vars)) %>% unique()
    prefix <-
      if (stat_type %in% c("count", "share")) {
        ""
      } else{
        paste0(stat_var, "_")
      }
    ...
    if (all(group_vars != "keep_existing")) {
      so %<>% ungroup()
    }                                          # "keep_existing" is power-user option for srvyr::combine() groupings;
    if (all(!is.null(group_vars) &
            group_vars != "keep_existing")) {
      # -- otherwise the package ungroups before and afterward
      if (incl_na == FALSE) {
        so %<>% filter(if_all(all_of(group_vars), ~ !is.na(.)))
      }                    # Allows users to exclude w/o removing observations from the data object itself
      so %<>% srvyr::group_by(across(all_of(group_vars)))                                            # Apply grouping
    }
    if (!is.null(geographic_unit)) {
      so %<>% srvyr::group_by(!!as.name(geographic_unit), .add = TRUE)
    }
    if (stat_type == "count") {
      rs <- suppressMessages(
        cascade(
      ...
      rs <- suppressMessages(
        cascade(
          so,
          count := survey_total(na.rm = TRUE),
          !!paste0(prefix, "total") := survey_total(!!as.name(stat_var), na.rm =
                                                      TRUE),
          !!paste0(prefix, "median") := survey_median(!!as.name(stat_var), na.rm =
                                                        TRUE),
          !!paste0(prefix, "mean") := survey_mean(!!as.name(stat_var), na.rm =
                                                    TRUE),
          sample_size := srvyr::unweighted(dplyr::n()),
          .fill = "Total"
        )
      ...
      srvyrf_name <-
        as.name(paste0("survey_", stat_type))                                            # Specific srvyr function name
      rs <- suppressMessages(cascade(
        so,
        !!paste0(prefix, stat_type) := (as.function(!!srvyrf_name)(!!as.name(stat_var), na.rm =
                                                                     TRUE)),
        sample_size := srvyr::unweighted(dplyr::n()),
        .fill = "Total"
      ))
    ...
    rs %<>% setDT() %>%
      .[, grep("_se", colnames(.)) := lapply(.SD, function(x)
        x * 1.645), .SDcols = grep("_se", colnames(.))] %>%
      setnames(grep("_se", colnames(.)),
               stringr::str_replace(grep("_se", colnames(.), value = TRUE), "_se", "_moe"))
    if (!is.null(geographic_unit)) {
...
hhts_bulk_stat <-
  function(df,
    ...
    df <- lapply(group_var_list, list_stat) %>%
      lapply(
        FUN = function(y) {
          mutate(y, "var_name" = colnames(y)[1])
        }
